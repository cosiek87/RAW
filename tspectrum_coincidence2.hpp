...
Bool_t w_zakresie_511kev(UShort_t energia, UShort_t channel) {
	...
	case 0:
		if (energia > 2600 && energia < 3400) w_zakresie = false;
		break;

	case 1:
		if (energia > 4600 && energia < 5600) w_zakresie = false;
		break;

	case 2:
		if (energia > 2250 && energia < 3250) w_zakresie = false;
		break;

	case 3:
		if (energia > 1400 && energia < 2600) w_zakresie = false;
		break;

	case 4:
		if (energia > 1100 && energia < 2300) w_zakresie = false;
		break;

	case 5:
		if (energia > 3400 && energia < 4700) w_zakresie = false;
		break;
...
Bool_t w_zakresie_sigma(ULong64_t energia, UShort_t channel, std::vector < UShort_t > centroidy, std::vector < UShort_t > sigmy, Double_t ile_sigma) {
	Bool_t w_zakresie = true;
	if (energia > centroidy[channel] - ile_sigma * sigmy[channel] && energia < centroidy[channel] + ile_sigma * sigmy[channel]) w_zakresie = false;
	return w_zakresie;
}
...
void wektory_czasu_koincydencji(int liczba_pomiarow, std::vector < ULong64_t > wektor_czasu,
                                ...
                                int para_detektorow,
                                char* sciezka_do_pliku_zrodlowego) {
    ...
    Char_t nazwa_1[200] = "SDataR_na22_kalibracja_sasiedzi_0.root";
    ULong64_t czas;
...
bool wieksze_od_min(Int_t i, Int_t min_count_per_spectrum){
    return i>min_count_per_spectrum;
}
